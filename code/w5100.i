...
# 44 "/opt/microchip/xc8/v2.10/avr/avr/include/util/delay.h" 3
...
# 45 "/opt/microchip/xc8/v2.10/avr/avr/include/util/delay.h" 2 3
...
unsigned char W51_config(W5100_CFG *pcfg)
{
    if (pcfg == 0) return 0xff;

    W51_write(0x0001 + 0, pcfg->gtw_addr[0]);
    W51_write(0x0001 + 1, pcfg->gtw_addr[1]);
    W51_write(0x0001 + 2, pcfg->gtw_addr[2]);
    W51_write(0x0001 + 3, pcfg->gtw_addr[3]);
    _delay_ms(1);

    W51_write(0x0009 + 0, pcfg->mac_addr[0]);
    W51_write(0x0009 + 1, pcfg->mac_addr[1]);
    W51_write(0x0009 + 2, pcfg->mac_addr[2]);
    W51_write(0x0009 + 3, pcfg->mac_addr[3]);
    W51_write(0x0009 + 4, pcfg->mac_addr[4]);
    W51_write(0x0009 + 5, pcfg->mac_addr[5]);
    _delay_ms(1);

    W51_write(0x0005 + 0, pcfg->sub_mask[0]);
    W51_write(0x0005 + 1, pcfg->sub_mask[1]);
    W51_write(0x0005 + 2, pcfg->sub_mask[2]);
    W51_write(0x0005 + 3, pcfg->sub_mask[3]);
    _delay_ms(1);

    W51_write(0x000F + 0, pcfg->ip_addr[0]);
    W51_write(0x000F + 1, pcfg->ip_addr[1]);
    W51_write(0x000F + 2, pcfg->ip_addr[2]);
    W51_write(0x000F + 3, pcfg->ip_addr[3]);
    _delay_ms(1);

    W51_write(0x001A, 0x55);
    W51_write(0x001B, 0x55);
