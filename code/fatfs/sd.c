...
//macros
#define disableSD() SD_CS_PORT.OUTSET = SD_CS_bm; clockSPI()
#define enableSD() SD_CS_PORT.OUTCLR = SD_CS_bm
...
static void initSD( void) {
    SD_CS_PORT.DIRSET = SD_CS_bm;       //CS line is output
    SD_CD_PORT.DIRCLR = SD_CD_bm;        //CD line is input

    #ifdef SD_LED_TRIS
    SD_LED_TRIS=0;
    #endif

    SD_CS_PORT.OUTSET = SD_CS_bm;  // initially keep the SD card disabled

    sd_bytesread=0;
    init=1;

    // init the SPI module for a slow (safe) clock speed first
    SD_SPI.CTRL=(SD_SPI.CTRL & (~SPI_PRESCALER_gm)) | SPI_PRESCALER_DIV64_gc;
} 
...
int sd_init( void) {
    ...
    if((r & (1 << R1_ILL_COMMAND)) == 0) {
        ...
        if((r & (1 << R1_ILL_COMMAND)) == 0) {
			...
			printf_P(PSTR("SD v1 card\r\n"));
        }
        else {
			printf_P(PSTR("MMC card\r\n"));
            /* MMC card */
        }
    }
...
int sd_readSECTOR( LBA a, char *p) {
    ...
    if ( r == 0) {   // check if command was accepted
        ...
        if ( i != R_TIMEOUT) {
			...
			printf_P(PSTR("readSECTOR %ld R_TIMEOUT return -2\r\n"),a);
			readlock=0;
            return 0;
        }
    } // command accepted
	else {
		printf_P(PSTR("readSECTOR  %ld cmd rejected return -1\r\n"),a);
		readlock=0;	
        return 0;
	}
...
int sd_writeSECTOR( LBA a, const char *p) {
    ...
    if ( r == 0) {     // check if command was accepted
        ...
        if ( (r & 0xf) == DATA_ACCEPT) {
            ...
            r = FAIL;
            printf_P(PSTR("writeSECTOR %ld failed r=%d\n"),a,r);
		}
    } // command accepted
	else {
		printf_P(PSTR("writeSECTOR %ld cmd rejected r=%d\n"),a,r);
	}
