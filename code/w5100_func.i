...
# 3 "w5100/w5100_func.c" 2
# 1 "/opt/microchip/xc8/v2.10/avr/avr/include/stdio.h" 1 3
# 45 "/opt/microchip/xc8/v2.10/avr/avr/include/stdio.h" 3
    ...
    unsigned int tcp_port)
{
 ...
 sockaddr = (0x0400 +(sock*0x0100));

 if (W51_read(sockaddr + 0x0003) == 0x00)
 {
  CloseSocket(sock);

 }

 W51_write(sockaddr + 0x0000, eth_protocol);
 W51_write(sockaddr + 0x0004, ((tcp_port & 0xFF00) >> 8));
 W51_write(sockaddr + 0x0004 + 1, (tcp_port & 0x00FF));
 W51_write(sockaddr + 0x0001, 0x01);

 while (W51_read(sockaddr + 0x0001)) ;
 _delay_ms(1);




 if (W51_read(sockaddr + 0x0003) == 0x22)
  {
   retval = sock;
  }
 else
...
void CloseSocket(unsigned char sock)
{
 ...
 W51_write(sockaddr + 0x0001, 0x10);
 while (W51_read(sockaddr + 0x0001)) ;
}
...
void DisconnectSocket(unsigned char sock)
{
 ...
 W51_write(sockaddr + 0x0001, 0x08);
 while (W51_read(sockaddr + 0x0001)) ;
}
...
unsigned char Listen(unsigned char sock)
{
 ...
 sockaddr = (0x0400 +(sock*0x0100));
 if (W51_read(sockaddr + 0x0003) == 0x13)
 {
  W51_write(sockaddr + 0x0001, 0x02);
  while (W51_read(sockaddr + 0x0001)) ;

  if (W51_read(sockaddr + 0x0003) == 0x14)
   retval = 0x00;
  else
   CloseSocket(sock);
 }
...
sockaddr = (0x0400 +(sock*0x0100));
 ...
 txsize = W51_read(sockaddr + 0x0020);
 txsize =
     (((txsize & 0x00FF) << 8) +
      W51_read(sockaddr + 0x0020 + 1));
 ...
 ptr = W51_read(sockaddr + 0x0024);
 offaddr =
     (((ptr & 0x00FF) << 8) +
      W51_read(sockaddr + 0x0024 + 1));

 while (buflen) {
  buflen--;
  realaddr = (0x4000 + (0x0800 * sock)) + (offaddr & 0x07FF);
 ...
 W51_write(sockaddr + 0x0024, (offaddr & 0xFF00) >> 8);
 W51_write(sockaddr + 0x0024 + 1, (offaddr & 0x00FF));

 W51_write(sockaddr + 0x0001, 0x20);
...
unsigned int buflen)
{
 ...
 sockaddr = (0x0400 +(sock*0x0100));
 ptr = W51_read(sockaddr + 0x0028);
 offaddr = (((ptr & 0x00FF) << 8) + W51_read(sockaddr + 0x0028 + 1));
 ...
 *buf = '\0';
 ...
 W51_write(sockaddr + 0x0028, (offaddr & 0xFF00) >> 8);
 W51_write(sockaddr + 0x0028 + 1, (offaddr & 0x00FF));


 W51_write(sockaddr + 0x0001, 0x40);
 _delay_us(5);
...
unsigned int ReceivedSize(unsigned char sock)
{
 ...
 return 0;
 sockaddr = (0x0400 +(sock*0x0100));
 val = W51_read(sockaddr + 0x0026) & 0xff;
 val = (val << 8) + W51_read(sockaddr + 0x0026 + 1);
 return val;
}
