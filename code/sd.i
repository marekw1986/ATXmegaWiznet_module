...
int sd_getCD();
int sd_getWP();
int sd_readSECTOR ( LBA, char *);
int sd_writeSECTOR( LBA, const char *);
# 11 "fatfs/sd.c" 2
# 1 "fatfs/../wdt_driver.h" 1
# 74 "fatfs/../wdt_driver.h"
...
# 181 "fatfs/sd.c"
 ...
 r = sendSDCmd(0x08, 0x1aa);
    if((r & (1 << 2)) == 0) {
        writeSPI( 0xFF);
        writeSPI( 0xFF);
        if((writeSPI( 0xFF) & 0x01) == 0) return 2;
        if(writeSPI( 0xFF) != 0xaa) return 3;
...
# 208 "fatfs/sd.c"
    ...
    for( i = 0; ; ++i) {
        if(sd_raw_card_type & ((1 << 0) | (1 << 1))) {
            long arg = 0;
            if(sd_raw_card_type & (1 << 1)) arg = 0x40000000;
            sendSDCmd(0x37, 0);
            r = sendSDCmd(0x29, arg);
        }
        else {
            r = sendSDCmd(0x01, 0);
        }
...
# 237 "fatfs/sd.c"
        ...
        if(writeSPI( 0xFF) & 0x40) sd_raw_card_type |= (1 << 2);
...
# 324 "fatfs/sd.c"
    ...
    if(sd_raw_card_type & (1 << 2)) r = sendSDCmd( 17, ( a << 9)/512);
    else r = sendSDCmd( 17, ( a << 9));
...
# 354 "fatfs/sd.c" 3
           (__extension__({static const char __c[] __attribute__((__progmem__)) = (
# 354 "fatfs/sd.c"
           "readSECTOR %ld R_TIMEOUT return -2\r\n"
# 354 "fatfs/sd.c" 3
           ); &__c[0];}))
# 354 "fatfs/sd.c"
                                                         ,a);
...
# 360 "fatfs/sd.c" 3
          (__extension__({static const char __c[] __attribute__((__progmem__)) = (
# 360 "fatfs/sd.c"
          "readSECTOR  %ld cmd rejected return -1\r\n"
# 360 "fatfs/sd.c" 3
          ); &__c[0];}))
# 360 "fatfs/sd.c"
                                                            ,a);
...
int sd_writeSECTOR( LBA a, const char *p) {
    ...
    if(sd_raw_card_type & (1 << 2)) r = sendSDCmd( 24, ( a << 9)/512);
    else r = sendSDCmd( 24, ( a << 9));
...
# 414 "fatfs/sd.c" 3
          (__extension__({static const char __c[] __attribute__((__progmem__)) = (
# 414 "fatfs/sd.c"
          "writeSECTOR %ld cmd rejected r=%d\n"
# 414 "fatfs/sd.c" 3
          ); &__c[0];}))
# 414 "fatfs/sd.c"
                                                     ,a,r);
